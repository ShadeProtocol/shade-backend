...
export const isApiKeyToken = (token) => token.startsWith(API_KEY_PREFIX);
export const hashApiKey = (rawKey) => crypto.createHash('sha256').update(rawKey).digest('hex');
export const generateApiKeyMaterial = () => {
    const randomPart = Array.from({ length: API_KEY_RANDOM_LENGTH }, () => KEY_ALPHABET[crypto.randomInt(0, KEY_ALPHABET.length)]).join('');
