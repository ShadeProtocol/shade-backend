import { createInvoice, getInvoice, getInvoiceWithMerchant, listInvoices, voidInvoice, } from '../services/invoice.services.js';
...
export const listInvoicesController = async (req, res) => {
    ...
    const { filters, pagination, errors } = parseInvoiceListQuery(req.query);
    if (Object.keys(errors).length > 0) {
        res.status(400).json({ error: 'Validation failed', errors });
        return;
    }
...
export const getInvoicePdfController = async (req, res) => {
    const merchant = req.merchant;
    if (!merchant) {
        res.status(401).json({ error: 'Unauthorized' });
        return;
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="invoice-${invoice.paymentSlug}.pdf"`);
