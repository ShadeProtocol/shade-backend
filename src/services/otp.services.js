...
export const hashOtp = async (code) => bcrypt.hash(code, BCRYPT_ROUNDS);
export const verifyOtpHash = async (code, hash) => bcrypt.compare(code, hash);
const getLastOtpSentAt = (expiresAt) => new Date(expiresAt.getTime() - OTP_EXPIRY_MS);
/**
 ...
 */
export const verifyEmailOtp = async (merchantId, code) => {
 ...
 */
export const resendEmailOtp = async (merchantId) => {
    const merchant = await prisma.merchant.findUnique({
        where: { id: merchantId },
    });
    if (!merchant) {
