 ...
 */
export const sanitizeMerchant = (merchant) => ({
    id: merchant.id,
...
export const listMerchants = async (limit, offset) => {
    try {
 ...
 */
export const registerMerchant = async (merchantId, data) => {
    const merchant = await prisma.merchant.findUnique({
        where: { id: merchantId },
    });
    if (!merchant) {
 ...
 */
export const generateMerchantSigningKey = async (id) => {
    const merchant = await prisma.merchant.findUnique({ where: { id } });
    if (!merchant) {
        throw new AppError(404, 'Merchant not found');
    }
    ...
    console.info(`[merchant] signing key ${merchant.merchantKey ? 'rotated' : 'created'} for merchant ${id}`);
...
export const updateMyProfile = async (id, data) => {
    const updateData = {};
    const textFields = ['firstName', 'lastName', 'businessName', 'category', 'description'];
