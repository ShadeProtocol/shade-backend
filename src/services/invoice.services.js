...
import { parseAmount, } from '../utils/invoice.validation.js';
...
export const createInvoice = async (merchantId, data) => {
    const amount = parseAmount(data.amount);
    if (amount === null) {
        throw new AppError(400, 'amount must be a positive integer');
    }
    const status = data.isDraft ? InvoiceStatus.DRAFT : InvoiceStatus.PENDING;
...
export const listInvoices = async (merchantId, filters, pagination) => {
...
export const getInvoice = async (merchantId, id) => {
 ...
 */
export const getInvoiceWithMerchant = async (merchantId, id) => {
    const invoice = await prisma.invoice.findFirst({
        where: { id, merchantId },
...
export const voidInvoice = async (merchantId, id) => {
    const invoice = await prisma.invoice.findFirst({
        where: { id, merchantId },
    });
    if (!invoice) {
